...
## 七、5 层 token 与意图强制（增量并入）

> 蒸馏自 research-notes-01（design-system-architecture）+ batch-039。把现有三层 token 往上再扩两层，把"品味"翻译成可机检约束。

- **第 4 层 Pattern/composition**（数字版 Alexander pattern language）：empty state、pricing section、settings form、dashboard 作为带 schema 的模式——`context`、`tone`、`forbidden: ['gradient-background','floating-emoji','three-card-upsell']`、允许的子组件与必填内容。
- **第 5 层 Governance/constraint**：lint 规则、token 可见性、forbidden-token 清单、codemod、视觉回归阈值。
- **token 作为"策略句柄"**（带元数据，非纯别名）：
  ```json
  "color.text.danger": {
    "value": "{color.red.700}",
  ...
  ```
- **包边界隔离为硬约束**：`@acme/tokens/reference` 设为 build-time-only private，app 代码只能 import `semantic`；ESLint `no-reference-token-in-app` + Stylelint 禁 app 代码出现 hex/rgb/hsl。
- **关系型 spacing token**（强于纯数值）：`--gap-inline-icon-label: .35em`、`--gap-heading-to-body: .6em`、`--gap-section: 4rem`、`--gap-scene: clamp(5rem,12vw,10rem)`——AI 选 `--gap-scene` 比选 `py-24` 更可约束。
- **Elevation 物理隐喻分级**：flat / ruled(`0 1px 0`) / lifted(`0 4px 16px`) / popover(`0 18px 40px`) / dialog(`0 32px 80px`) / glow（仅 active/focus）。静态内容不得用 popover 以上。
- **Density 作顶层系统变量**：4–5 档（productive/standard/editorial/immersive），每档映射 controlGap/panelPadding/sectionGap/typeRole；**密度由 zone 决定，AI 不得为 section 自行发明密度**。
- **内容驱动布局选择**：`chooseLayoutPattern(content)`——workflow→annotatedProcess、data→dataLed、comparison→ledger(`grid-cols-[12ch_1fr]`)、brand→editorialSplit(标题占 7/12 列)、default→posterGrid；toolInterface 用真实截图+注解，不用装饰 dashboard。

**组件 API 意图强制**：拆 `variant` 为正交维度 `intent`/`emphasis`/`density`/`context`（`<Button intent="submit" emphasis="primary">`）；discriminated union 强制必填（warning callout 必须带 `mitigationAction` 否则不编译）；`className?: never` 默认关闭 escape hatch，需要时用 `UnsafeStyleOverride { reason; approvedBy; expiresOn }`；EmptyState 按 `cause`（no-records-yet / filtered-no-results / permission-limited）强制 domain + recovery action。

**Forbidden patterns 清单**（可直接做 banned-list / lint）：
- forbiddenUtilityPatterns：`from-purple-*`、`to-pink-*`、`backdrop-blur*`、`shadow-2xl`、`rounded-3xl`、`bg-gradient-to-*`
- forbiddenRawValues：`#8b5cf6`、`linear-gradient(135deg,#667eea,#764ba2)`、`0 20px 25px -5px rgb(0 0 0/.1)`
- forbiddenCompositions：`generic-hero-with-gradient-orbs`、`three-feature-cards-with-lucide-icons`、`glassmorphism-dashboard-card`、`fake-analytics-chart-with-random-bars`
- anti-pattern token 作诊断标签（不暴露给作者，仅让 lint 报错具体化）：`effect.aiSlop.glassCard`（检测 `backdrop-blur` + `bg-white/10` + `border-white/20`）、`composition.aiSlop.genericHero`
