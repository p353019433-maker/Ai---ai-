...
## 七、Aesthetic linting：把"品味"做成 CI（增量并入）

> 蒸馏自 research-notes-04（aesthetic-linting）+ batch-038/039。定位：检测 **AI-pattern risk / generic-UI risk**，不是证明 AI 作者身份；检测 pattern concentration，不是 intent。

**四层检测架构**：① Static JSX/TSX lint（className AST）② CSS/token lint（Stylelint）③ Rendered DOM + a11y（Playwright computed styles）④ Screenshot 视觉指标。

**ESLint 规则目录**（可直接落地）：
- `no-radius-monoculture`：`maxSameRadiusPerComponent: 4`、`maxSameRadiusPerFile: 8`；区分 container radius（高风险）vs avatar/pill `rounded-full`（正常）。
- `no-gradient-cliche`：gradient text = `bg-clip-text` + `text-transparent` + `bg-gradient-to-*`；bannedPairs = [from-blue-600,to-purple-600]/[from-indigo-500,to-pink-500]/[from-purple-500,to-pink-500]/[from-cyan-500,to-blue-500]；blur blob = `absolute`+`rounded-full`+`blur-*`+`opacity-*`。
- `no-generic-card-stack`（**评分模型**）：raw div +1；大 radius +2；border +1；大 shadow +2；`p-6/p-8` +1；半透明白底 +1；hover-lift +1；icon-badge 子元素 +2；处于 `grid md:grid-cols-3 gap-8` +2；`h3+p` +1。warn≥6，error≥9。
- 其他：`no-raw-design-values`（arbitrary `rounded-[20px]`/`bg-[#...]`/inline shadow）、`prefer-design-system-components`、`no-icon-badge-grid`、`no-ornamental-blur-blob`、`no-transition-all-garnish`、`no-placeholder-marketing-copy`、`require-aesthetic-metadata-for-new-stories`。
- 提取工具 `extractClassChunksFromExpression` 必须递归处理 `cn/clsx/cva/tv` 的 CallExpression、`&&`、三元、数组、对象 key、模板字符串。

**Screenshot 可量化指标**（DOM-assisted 优于纯像素，用 Playwright `getBoundingClientRect`+`getComputedStyle`）：radius top-share（area-weighted）+ entropy；shadow distribution（distinct recipe 数、重复 `0 20px 25px`）；spacing histogram（gap 桶 4/8/.../96，dominant share + entropy）；palette entropy（像素→LAB→k-means，grayscale share、accent area、与品牌色距离）；edge density（Sobel）；card silhouette 重复度（perceptual hash）；center-aligned 文本块占比；whitespace-to-content ratio；**layout signature**（序列化 `pill@y0.12 → heading center w0.70 → paragraph center → ctaRow count2 → cardGrid cols3 gap32`，匹配 generic hero 且无审批则升风险）。

**风险聚合公式**（可配置权重）：
```
risk = min(100,
  0.30*staticPattern + 0.20*tokenDrift + 0.25*visualMetric +
  0.15*repetitionDelta + 0.10*accessibilityOverlap)
warn=40, fail=75（按 surface 调：marketing fail=85, app fail=65）
```

**CI 七阶段**：inputs/config → static lint → CSS+token lint → build Storybook + Playwright 截图（1440×900 / 768×1024 / 390×844，light+dark+reduced-motion）→ visual metrics → risk 聚合（SARIF/PR comment/HTML）→ review。治理：**与 baseline 比较而非绝对阈值**（marketplace 可用 card grid，dashboard 不行）；exception 必带 `owner`+`rationale`+Figma 链接+`expires`（`// aesthetic-disable-next-line ... -- DS-1234 until 2026-09-01`）；banned-pattern 入流程=收集 3–5 例 → report-only 跑 1–2 sprint → 调 false positive → 升 warn/error。statuses：banned / warn / deprecated / allowed-with-token / allowed-with-approval / experimental。

**三个快速判定测试**：
- **终极测试**："换个 Logo，这页还能属于任何 AI 创业公司吗？若能，仍是 slop。"
- **squint test**：模糊页面，主焦点仍可辨则通过。
- **去色测试**：去掉颜色，层级还成立吗？不成立则颜色在做过多装饰工作。
- 生成后清单：移除一层装饰 / 添加一个内容特定细节 / section 感觉相同则改节奏 / 包含 non-happy-path 状态。
